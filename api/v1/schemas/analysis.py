...
class AnalyzeRequest(BaseModel):
    ...
    stock_code: Optional[str] = Field(None, description="单只股票代码", json_schema_extra={"example": "600519"})
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "stock_code": "600519",
                "stock_name": "贵州茅台",
                "report": {"summary": {"sentiment_score": 75, "operation_advice": "持有"}},
...
class TaskStatus(BaseModel):
    ...
    result: Optional[AnalysisResultResponse] = Field(None, description="分析结果（仅在 completed 时存在）")
    error: Optional[str] = Field(None, description="错误信息（仅在 failed 时存在）")
    stock_name: Optional[str] = Field(None, description="股票名称")
    original_query: Optional[str] = Field(None, description="用户原始输入")
    selection_source: Optional[str] = Field(None, description="选择来源", pattern=SELECTION_SOURCE_PATTERN)
...
class TaskListResponse(BaseModel):
    ...
    model_config = ConfigDict(json_schema_extra={"example": {"total": 3, "pending": 1, "processing": 2, "tasks": []}})
