...
class ConfigEnvCompatibilityTestCase(unittest.TestCase):
    ...
    def test_load_from_env_keeps_default_behavior_without_tickflow_api_key(
        ...
        self.assertEqual(
            ...
            "tencent,tickflow,akshare_sina,efinance,akshare_em",
        )

    @patch("src.config.setup_env")
    @patch.object(Config, "_parse_litellm_yaml", return_value=[])
    def test_load_from_env_appends_tushare_to_default_realtime_priority_when_token_present(
        self, _mock_parse_litellm_yaml, _mock_setup_env
    ):
        with patch.dict(
            os.environ,
        ...
        self.assertEqual(
            config.realtime_source_priority,
            "tencent,tickflow,akshare_sina,efinance,akshare_em,tushare",
        )
