...
class TestPipelinePrefetchBehavior(unittest.TestCase):
    ...
    def _build_pipeline(process_result):
        ...
        pipeline._build_candidate_pool = MagicMock(
            side_effect=lambda stock_codes, dry_run=False: {
                "original_stock_codes": list(stock_codes),
                "mx_candidate_pool": list(stock_codes),
                "portfolio_pool": [],
                "final_candidate_pool": list(stock_codes),
                "mx_xuangu_enabled": False,
                "mx_xuangu_reason": "test",
                "mx_xuangu_source": "test",
                "portfolio_forced_included": False,
                "fallback_used": True,
                "dry_run": dry_run,
            }
        )
        pipeline.fetcher_manager.prefetch_realtime_quotes = MagicMock(return_value=0)
