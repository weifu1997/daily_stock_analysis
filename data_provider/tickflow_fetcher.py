...
class TickFlowFetcher(BaseFetcher):
    ...
    def get_realtime_quote(self, stock_code: str) -> Optional[UnifiedRealtimeQuote]:
        """Fetch per-stock realtime quote from TickFlow."""
        client = self._get_client()
        if client is None:
            return None
        ...
        # TickFlow symbol convention for A-share stocks prefers exchange suffix.
        ...
        try:
            ...
            logger.info("[TickFlowFetcher] 个股实时查询失败: %s", exc)
            return None
        ...
        if change_amount is None and current is not None and prev_close is not None:
            change_amount = current - prev_close

        change_pct = self._ratio_to_percent(ext.get("change_pct"))
        if change_pct is None and current is not None and prev_close not in (None, 0):
            change_pct = (current - prev_close) / prev_close * 100.0

        amplitude = self._ratio_to_percent(ext.get("amplitude"))
        if amplitude is None and prev_close not in (None, 0) and high is not None and low is not None:
            amplitude = (high - low) / prev_close * 100.0

        volume_ratio = self._safe_float(ext.get("volume_ratio"))
        turnover_rate = self._safe_float(ext.get("turnover_rate"))
        pe_ratio = self._safe_float(ext.get("pe_ratio"))
        pb_ratio = self._safe_float(ext.get("pb_ratio"))
        total_mv = self._safe_float(ext.get("total_mv"))
        circ_mv = self._safe_float(ext.get("circ_mv"))
