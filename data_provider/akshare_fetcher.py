        ...
        try:
            ...
            for attempt in range(1, 4):
                try:
                    ...
                    if attempt < 3:
                        logger.warning(f"[API重试] ak.stock_cyq_em 第 {attempt} 次失败: {inner_exc}")
                        _time.sleep(1.0 * attempt)
            if df is None:
                raise last_error if last_error is not None else RuntimeError("ak.stock_cyq_em 返回 None")
            ...
            elif "proxy" in lowered or "dns" in lowered or "name or service not known" in lowered or "connection refused" in lowered:
                reason = "environment_or_proxy_network"
            elif "权限" in err or "access permission" in lowered or "not authorized" in lowered:
                reason = "permission_or_api_error"
            elif "too many" in lowered or "429" in lowered or "频率" in err:
                reason = "rate_limited"
            elif "empty" in lowered or "返回空" in err:
                reason = "empty_response"
            elif stock_code and stock_code[:2] in {"60", "68", "30", "00", "83", "87", "88", "51", "52", "56", "58"}:
                reason = "specific_code_trigger"
            else:
                reason = "unknown_akshare_error"
            logger.error(f"[API错误][{reason}] 获取 {stock_code} 筹码分布失败: {e}")
            return None
