...
class DataFetcherManager:
        ...
        for source in source_priority:
            ...
            try:
                ...
                if source == "efinance":
                    ...
                    # 尝试 TickFlowFetcher 个股实时数据源（若可用）
                    tickflow_fetcher = self._get_tickflow_fetcher()
                    if tickflow_fetcher is not None and hasattr(tickflow_fetcher, 'get_realtime_quote'):
                        quote = self._call_fetcher_method(tickflow_fetcher, 'get_realtime_quote', stock_code)
    ...
    def get_chip_distribution(
        ...
        1. Tushare
        2. AkShare / 东方财富
        3. OHLCV 估算筹码（estimated_ohlcv）

        每次失败都会分类记录到 chip_source_events，便于后续诊断。
        ...
        for fetcher_name, fetcher_names, source_key in source_order:
            ...
            if not hasattr(fetcher, 'get_chip_distribution'):
                _record_chip_event(fetcher_name, "missing", "method_not_implemented")
                continue
        ...
        # 真实源失败后，尝试 OHLCV 估算兜底
        try:
            estimated_daily_df = daily_df
            estimated_current_price = current_price

            if estimated_current_price is None:
                quote = self.get_realtime_quote(stock_code, log_final_failure=False)
                if quote is not None:
                    estimated_current_price = getattr(quote, 'current_price', None) or getattr(quote, 'close', None)

            if estimated_daily_df is None or estimated_daily_df.empty:
                for fetcher in self._get_fetchers_snapshot():
                    if not hasattr(fetcher, 'get_daily_data'):
                        continue
                    try:
                        estimated_daily_df = self._call_fetcher_method(fetcher, 'get_daily_data', stock_code, days=120)
                        if estimated_daily_df is not None and not estimated_daily_df.empty:
                            break
                    except Exception as daily_exc:
                        logger.debug(f"[筹码分布] {stock_code} 日线兜底失败 (fetcher={getattr(fetcher, 'name', '')}): {daily_exc}")
                        continue

            if estimated_daily_df is not None and not estimated_daily_df.empty and estimated_current_price is not None:
                estimated = estimate_chip_distribution(estimated_daily_df, stock_code=stock_code, current_price=float(estimated_current_price), window=120)
                if estimated is not None:
                    _record_chip_event("EstimatedChipEstimator", "success", f"sample_days={estimated.sample_days}")
                    logger.info(f"[筹码分布] {stock_code} 使用估算兜底成功 (source={estimated.source}, confidence={estimated.confidence:.2f})")
                    return estimated
                _record_chip_event("EstimatedChipEstimator", "empty", "estimate_returned_none")
            else:
                _record_chip_event("EstimatedChipEstimator", "missing", "daily_data_or_current_price_missing")
        except Exception as e:
            logger.warning(f"[筹码分布] {stock_code} 估算兜底失败: {e}")
            _record_chip_event("EstimatedChipEstimator", "failed", str(e))
    ...
    def get_stock_name(self, stock_code: str, allow_realtime: bool = True) -> Optional[str]:
        ...
        try:
            ...
            mx_cached_name = get_cached_stock_name(stock_code)
        ...
        # 4. TickFlow 用作轻量补全；如果没有就不再打更多后查
        try:
            tickflow_fetcher = self._get_tickflow_fetcher()
            if tickflow_fetcher is not None and hasattr(tickflow_fetcher, 'get_stock_name'):
                name = self._call_fetcher_method(tickflow_fetcher, 'get_stock_name', stock_code)
                if is_meaningful_stock_name(name, stock_code):
                    self._cache_stock_name(stock_code, name)
                    logger.info(f"[股票名称] 从 TickFlowFetcher 获取: {stock_code} -> {name}")
                    return name
        except Exception as e:
            logger.debug(f"[股票名称] TickFlowFetcher 获取失败: {e}")

        # 5. 兜底：仅在确实无名时，尝试少量其他数据源；Tushare 仍放最后
        from .akshare_fetcher import _is_us_code
        ...
        for fetcher in fetchers:
            if not hasattr(fetcher, 'get_stock_name'):
                continue
            if fetcher.name in {"TushareFetcher", "PytdxFetcher"}:
                continue
        ...
        # 6. 最后才考虑 Tushare（尽量避免）
        for fetcher in fetchers:
            if getattr(fetcher, 'name', '') != 'TushareFetcher' or not hasattr(fetcher, 'get_stock_name'):
                continue
            try:
                name = self._call_fetcher_method(fetcher, 'get_stock_name', stock_code)
                if is_meaningful_stock_name(name, stock_code):
