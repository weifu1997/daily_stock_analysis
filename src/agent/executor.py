...
{{
    ...
    "dashboard": {{
        ...
        "data_perspective": {{
            ...
            "chip_structure": {{"profit_ratio": 0, "avg_cost": 0, "concentration": 0, "chip_health": "", "source": "", "confidence": 0, "method": ""}}
        }},
        "intelligence": {{
            "latest_news": "",
            "risk_alerts": [],
...
class AgentExecutor:
    ...
    def _build_user_message(self, task: str, context: Optional[Dict[str, Any]] = None) -> str:
        ...
        if context:
            ...
            if context.get("fundamental_quality"):
                parts.append(f"\n[系统已获取的基本面质量信号]\n{json.dumps(context['fundamental_quality'], ensure_ascii=False)}")
