...
class NotificationService(
        ...
        if self._report_summary_only:
            ...
            for result in sorted_results:
                ...
                report_lines.extend([
                    ...
                    "",
                    f"**{labels['action_advice_label']}：{localize_operation_advice(result.operation_advice, report_language)}（次日执行建议）** | "
                ...
                if hasattr(result, 'buy_reason') and result.buy_reason:
                    report_lines.extend([
                        f"**💡 操作理由（用于次一交易日执行）**：{result.buy_reason}",
                        "",
                    ])
        ...
        if not self._report_summary_only:
            for result in sorted_results:
                ...
                report_lines.extend([
                    ...
                    "",
                    f"**{signal_emoji} {signal_text}（次日执行建议）** | {localize_trend_prediction(result.trend_prediction, report_language)}",
                    "",
                ...
                if pos_advice:
                    report_lines.extend([
                        ...
                        "|---------|---------|",
                        f"| 🆕 **{labels['no_position_label']}** | {pos_advice.get('no_position', localize_operation_advice(result.operation_advice, report_language) + '（次日执行建议）')} |",
                        f"| 💼 **{labels['has_position_label']}** | {pos_advice.get('has_position', labels['continue_holding'] + '（次日执行建议）')} |",
                        "",
                    ])
                ...
                if data_persp:
                    ...
                    if chip_data:
                        ...
                        source = chip_data.get('source', 'N/A')
                        confidence = chip_data.get('confidence', 'N/A')
                        method = chip_data.get('method', 'N/A')
                        report_lines.extend([
                ...
                mx_enrichment = getattr(result, 'mx_enrichment', None) or dashboard.get('mx_enrichment', {}) if dashboard else {}
                mx_data_summary = mx_enrichment.get('mx_data_summary', {}) if isinstance(mx_enrichment, dict) else {}
                if mx_data_summary:
                    report_lines.extend([
                        "### 🤖 妙想数据摘要",
                        "",
                        f"- 已启用: {mx_data_summary.get('mx_data_enabled')}",
                        f"- 查询成功: {mx_data_summary.get('mx_data_ok')}",
                    ])
                    if mx_data_summary.get('error'):
                        report_lines.append(f"- 错误: {mx_data_summary.get('error')}")
                    raw = mx_data_summary.get('raw')
                    if raw is not None:
                        report_lines.append(f"- 返回预览: {str(raw)[:300]}")
                    report_lines.append("")
    ...
    def generate_wechat_dashboard(self, results: List[AnalysisResult]) -> str:
        ...
        if self._report_summary_only:
            ...
            for result in sorted_results:
                ...
                # 标题行：信号等级 + 股票名称
                lines.append(f"### {signal_emoji} **{signal_text}（次日执行建议）** | {stock_name}({result.code})")
                lines.append("")
                ...
                if pos_advice:
                    ...
                    if no_pos:
                        lines.append(f"🆕 {labels['no_position_label']}: {no_pos[:50]}（次日执行建议）")
                    if has_pos:
                        lines.append(f"💼 {labels['has_position_label']}: {has_pos[:50]}（次日执行建议）")
                    lines.append("")
    ...
    def generate_wechat_summary(self, results: List[AnalysisResult]) -> str:
        ...
        for result in sorted_results:
            ...
            lines.append(
                f"**{localize_operation_advice(result.operation_advice, report_language)}（次日执行建议）** | "
                f"{labels['score_label']}:{result.sentiment_score} | "
...
class NotificationBuilder:
    ...
    def build_stock_summary(results: List[AnalysisResult]) -> str:
        ...
        for r in sorted(results, key=lambda x: x.sentiment_score, reverse=True):
            ...
            lines.append(
                f"{emoji} {name}({r.code}): {localize_operation_advice(r.operation_advice, report_language)}（次日执行建议） | "
                f"{labels['score_label']} {r.sentiment_score}"
            )
