...
class SearchService:
    ...
    _FINANCIAL_QUERY_HINTS = (
        "股票", "股价", "行情", "新闻", "公告", "研报", "政策", "行业", "事件", "影响",
        "业绩", "财报", "减持", "增持", "评级", "目标价", "资金流", "龙虎榜", "分红", "回购",
        "latest news", "earnings", "report", "analysis", "announcement", "industry", "policy",
    )
    ...
    def __init__(
        ...
        # 初始化搜索引擎（按优先级排序）
        # 1. SearXNG（自建实例优先；未配置时可自动发现公共实例）
    ...
    def _is_financial_query(self, query: str, stock_code: str = "", stock_name: str = "") -> bool:
        """Heuristic classifier for finance-related search queries."""
        text = f"{query or ''} {stock_code or ''} {stock_name or ''}".strip().lower()
        if not text:
            return False
        if stock_code or stock_name:
            return True
        return any(hint.lower() in text for hint in self._FINANCIAL_QUERY_HINTS)

    def _mx_search_to_response(self, query: str, raw: Any, *, provider: str = "mx-search", max_results: int = 5) -> SearchResponse:
        """Convert MX raw payload into SearchResponse.

        兼容妙想 skill 的真实返回结构：
        - 顶层 items
        - data/result 包裹
        - llmSearchResponse / searchResponse
        - llmSearchResponse.data / searchResponse.data
        - news-search 常见文本/对象混合结构
        """

        def unwrap(obj: Any) -> Any:
            current = obj
            seen = 0
            while isinstance(current, dict) and seen < 6:
                seen += 1
        ...
        def extract_items(obj: Any) -> List[Dict[str, Any]]:
            ...
            # 常见包装层级优先展开：items / dataList / list / records / data
            for key in (
                "items",
                "dataList",
                "list",
            ...
            for key in (
                ...
                value = obj.get(key)
                if isinstance(value, dict):
                    nested_items = extract_items(unwrap(value))
                    if nested_items:
                        return nested_items
            ...
            # 有些返回会把正文直接放在 trunk / content / answer / summary
            for key in ("trunk", "content", "answer", "summary"):
                value = obj.get(key)
                if isinstance(value, list):
                    nested_items = [item for item in value if isinstance(item, dict)]
                    if nested_items:
        ...
        for item in items[:max_results]:
            title = str(item.get("title") or item.get("name") or item.get("headline") or item.get("subject") or "").strip()
            snippet = str(
                item.get("summary")
                or item.get("snippet")
                or item.get("content")
            ...
            source = str(item.get("source") or item.get("insName") or provider).strip() or provider
            published_date = (
            ...
            results.append(
                SearchResult(
                    ...
                    snippet=snippet or title or query,
                    url=url,
                    source=source,
    ...
    def _mx_search_with_timeout(
        ...
        **kwargs: Any,
        ...
        """Execute MX search with timeout and structured logging."""
        ...
        future = executor.submit(self.mx_client.search, query, max_results=max_results, days=days, **kwargs)
        try:
        ...
        mx_response.search_time = elapsed
        if mx_response.success and len(mx_response.results) >= self.mx_search_min_results:
            logger.info("[mx-search] 主搜命中: label=%s query='%s' results=%s elapsed=%.2fs", route_label, query, len(mx_response.results), elapsed)
            return mx_response

        if mx_response.success and mx_response.results:
            logger.info("[mx-search] 结果不足触发 fallback: label=%s query='%s' results=%s threshold=%s elapsed=%.2fs", route_label, query, len(mx_response.results), self.mx_search_min_results, elapsed)
    ...
    def search_stock_events(
        ...
        mx_response = self._mx_search_with_timeout(
            ...
            route_label="search_stock_events",
            stock_code=stock_code,
            stock_name=stock_name,
        )
        if mx_response.success and mx_response.results and len(mx_response.results) >= self.mx_search_min_results:
