    ...
    def _extract_stream_text(self, chunk: Any) -> str:
        ...
        def _extract_text(payload: Any) -> Any:
            if isinstance(payload, dict):
                ...
                reasoning_content = getattr(payload, "reasoning_content", None)
            else:
                content = None
        ...
        content: Any = _extract_text(delta)
        if not content:
...
### 仓位语义约束（强制）
- 只有当该股票明确属于当前持仓池时，才允许使用“加仓 / 减仓 / 清仓”这类仓位动作词
- 如果该股票不在持仓池中，严禁使用“加仓 / 减仓 / 清仓”
- 非持仓股票只能使用：买入 / 持有 / 观望 / 卖出 / 回避 / 等待
- 如果无法确认是否持仓，默认按非持仓处理
    ...
    def _build_integrity_complement_prompt(self, missing_fields: List[str], report_language: str = "zh") -> str:
        ...
        for f in missing_fields:
            if f == "sentiment_score":
                ...
                lines.append("- operation_advice: 仅持仓股可用加仓/减仓/清仓；非持仓股只能用买入/持有/观望/卖出/回避/等待")
            elif f == "analysis_summary":
