...
**AI-powered stock analysis system for A-shares**

Analyze your watchlist daily → generate a decision dashboard → push to multiple channels (企业微信/飞书/邮箱/Email/WeChat Work/Feishu)
...
| Market | A-shares | A-shares only |
| Search | Smart Autocomplete (MVP) | **[Beta]** Home search supports code/name/pinyin/aliases; the local index covers A-shares and can be refreshed from Tushare or AkShare data |
...
- **Multi-market coverage**: the local index now covers **A-shares**. It can be regenerated from either Tushare or AkShare source data when needed.
- **Graceful fallback**:
