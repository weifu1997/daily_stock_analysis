...
- 📊 **盘中市场统计链路改为 TickFlow/Efinance 优先**（未发布）— 盘中市场涨跌统计不再默认优先触发 Tushare `rt_k`，manager 现在先尝试 TickFlow，随后回退到非 Tushare 数据源，Tushare 仅保留收盘后日线统计或最后兜底，避免 `rt_k` 限频告警污染默认生产链路。
